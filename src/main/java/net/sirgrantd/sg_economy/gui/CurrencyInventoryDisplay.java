...
public class CurrencyInventoryDisplay {
    ...
    public static void eventHandler(ScreenEvent.Render.Post event) {
        ...
        if (isInventory || isCreative || isCurios) {
            ...
            int yOffsetImage = isCreative ? -76 + ClientConfig.yDisplayCurrency : -26 + ClientConfig.yDisplayCurrency;

            int xOffsetText = 90 + ClientConfig.xDisplayCurrency;
            int yOffsetText = isCreative ? -67 + ClientConfig.yDisplayCurrency : -17 + ClientConfig.yDisplayCurrency;

            ResourceLocation DisplayImage = isMagicCoins ? ImageDisplayRender.DISPLAY_VIEW_MAGIC_COINS
                    : ImageDisplayRender.DISPLAY_VIEW_DEFAULT;

            ImageDisplayRender displayRender = new ImageDisplayRender(gui, xOffsetImage, yOffsetImage, DisplayImage);
            displayRender.renderWidget(event.getGuiGraphics(), 96, 24
        );
